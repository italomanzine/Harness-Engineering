...
6. **Backlog:** Use GitHub MCP para criar/atualizar cards de User Story em `italomanzine/Alexandria-UFSC`, trackear no Project 3 e definir `Status=Backlog`.
...
3. **Analyze:** Leia o card/Issue via GitHub MCP em `italomanzine/Alexandria-UFSC` e a spec em `specs/<slug>/spec.md`.
4. **Ready Gate:** Confirme que o card está no Project 3 com `Status=Ready`. Se não estiver, pare o fluxo.
5. **Kanban Start:** Ao iniciar desenvolvimento, mova o card para `In progress`.
6. **Branch:** Garanta que o trabalho não está na `main`/`master`; crie `feature/<slug>` a partir da `main` em `italomanzine/Alexandria-UFSC`.
...
15. **PR:** Só crie PR para `main` em `italomanzine/Alexandria-UFSC` se `.memory/last-evaluation.json` estiver `PASS`.
